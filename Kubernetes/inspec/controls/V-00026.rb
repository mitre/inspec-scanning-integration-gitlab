...
control 'V-00026' do
	...
	grep -i secure-port * 
	If the setting secure-port is set to 0 or is not found in the Kubernetes API manifest file, this is a finding.
"
    desc  'fix', "Edit the Kubernetes API Server manifest file in the /etc/kubernetes/manifests directory on the Kubernetes Master Node.  Set the argument --secure-port to value greater than 0.  
"
    impact 0.5
    tag severity: 'medium'
    ...
    describe file('/etc/kubernetes/manifests/kube-apiserver.manifest') do
        its('content') { should match '--secure-port' }
        its('content') { should_not match '--secure-port=0' }
    end
end