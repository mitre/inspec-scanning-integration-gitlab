...
control 'V-00036' do
    ...
    desc  'fix', "Edit any manifest files or kubelet config files that contain the feature-gates setting with DynamicAuditing set to true.  Set the flag to false or remove the DynamicAuditing setting completely.  Restart the kubelet service if the kubelet config file is changed."
    impact 0.5
    tag severity: 'medium'
    ...
    describe bash('cd /etc/kubernetes/manifests/ && grep -i feature-gates *') do
        its('stdout') { should_not match '- --feature-gates=DynamicAuditing=true' }
    end

    describe bash('cd /etc/sysconfig/ && grep -i feature-gates *') do
        its('stdout') { should_not match '- --feature-gates=DynamicAuditing=true' }
    end
end