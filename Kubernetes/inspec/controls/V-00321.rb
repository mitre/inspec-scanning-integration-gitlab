...
control 'V-00321' do
	...
	If the setting audit-log-path is not set in the Kubernetes API Server manifest file or it is set to to a valid path, this is a finding."
    ...
    GrepAuditLogPath = command("cd /etc/kubernetes/manifests/ && sudo grep -i audit-log-path *").stdout
    
    describe 'This test can only be performed by manual examination.' do
        skip "Manual Check: If the setting audit-log-path is not set in the Kubernetes API Server manifest file or it is set to to a valid path, this is a finding. Audit Log Path in Kubernetes Manifests:\n #{GrepAuditLogPath}"
    end
end