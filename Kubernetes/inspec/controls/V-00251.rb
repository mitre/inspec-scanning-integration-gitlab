...
control 'V-00251' do
	...
	If --basic-auth-file has a setting in the Kubernetes API server manifest file, this is a finding."
    desc  'fix', "Edit the Kubernetes APIi Server manifest file in the /etc/kubernetes/manifests directory on the Kubernetes Master Node.  Remove setting -basic-auth-file ."
    impact 0.7
    tag severity: 'high'
    ...
    describe file('/etc/kubernetes/manifests/kube-apiserver.manifest') do
        its('content') { should_not match '--basic-auth-file' }
    end
end