...
control 'V-00310' do
    ...
    desc  'rationale', ''
    desc  'check', "Review the Kubernetes Kubeadm conf files by using the command:
	stat -c %U:%G /usr/bin/kubeadm.conf| grep -v root:root
	If the command returns any non root:root file permissions, this is a finding."
    desc  'fix', "Change the ownership of the kubeadm.conf to root: root by executing the command:
	chown root:root /user/bin/kubeadm.conf"
    impact 0.5
    tag severity: 'medium'
