...
control 'V-00029' do
	...
	"
    desc  'fix', "Edit the Kubernetes Kubelet file in the/etc/sysconfig/ directory on the Kubernetes Master and Worker nodes.  Set the argument --authorization-mode to Webhook.  Restart each kubelet service after the change is made using the command:
    ...
    describe file('/etc/sysconfig/kubelet') do
        its('content') { should match '--authorization-mode=Webhook' }
    end
end