...
control 'V-00302' do
    ...
    describe bash('stat -c %U:%G /etc/kubernetes/admin.conf | grep -v root:root') do
        its('stdout') { should be_empty }
        its('stderr') { should eq '' }
    end

    describe bash('stat -c %U:%G /etc/kubernetes/scheduler.conf | grep -v root:root') do
        its('stdout') { should be_empty }
        its('stderr') { should eq '' }
    end

    describe bash('stat -c %U:%G /etc/kubernetes/controller-manager.conf | grep -v root:root') do
        its('stdout') { should be_empty }
        its('stderr') { should eq '' }
    end
end