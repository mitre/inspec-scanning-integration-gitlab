...
control 'V-00024' do
	...
	kubelet
 --read-only-port 
	If the setting insecure-port value is not set to 0 or is not set in the Kubernetes Kubelet, this is a finding."
    desc  'fix', "Edit the Kubernetes Kubelet file in the /etc/sysconfig directory on the Kubernetes Master Node.  Set the argument --read-only-port to 0.  
...
"
    ...
    describe file('/etc/sysconfig/kubelet') do
        its('content') { should match '--read-only-port=0' }
    end
end