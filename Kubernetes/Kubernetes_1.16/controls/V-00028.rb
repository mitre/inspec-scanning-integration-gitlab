...
control 'V-00028' do
	...
	more kubelet  
--anonymous-auth
	If the setting anonymous-auth is set to true or parameter not set in the Kubernetes Kubelet, this is a finding."
    desc  'fix', "Edit the Kubernetes Kubelet file in the/etc/sysconfig/ directory on the Kubernetes Master Node.  Set the argument --anonymous-auth to false.  Restart kubelet service using command:
service kubelet restart"
    ...
    describe file('/etc/sysconfig/kubelet') do
        its('content') { should match '--anonymous-auth=false' }
    end
end