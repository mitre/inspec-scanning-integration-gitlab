...
control 'V-00023' do
	...
	If the setting insecure-port is not set to 0 or is not found in the Kubernetes API server manifest file, this is a finding.
"
...
"
    ...
    describe file('/etc/kubernetes/manifests/kube-apiserver.manifest') do
        its('content') { should match '--insecure-port=0' }
    end
end