...
control 'V-00139' do
	...
	grep -i key-file * 
	If the setting etcd-keyfile is not set in the Kubernetes etcd manifest file, this is a finding."
    desc  'fix', "Edit the Kubernetes API Server manifest file in the /etc/kubernetes/manifests directory on the Kubernetes Master Node.  Set the value of --key-file to the certificate to the Approved Organizational Certificate.."
    impact 0.5
    tag severity: 'medium'
    ...
    describe file('/etc/kubernetes/manifests/etcd-main.manifest') do
        its('content') { should match 'etcd-keyfile' }
    end
end