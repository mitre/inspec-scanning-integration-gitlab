...
control 'V-00018' do
	...
	If the setting  authorization-mode is not set in the Kubernetes API Server mainifest file or is not set to Node, this is a finding."
    desc  'fix', "Edit the Kubernetes API Server manifest file in the /etc/kubernetes/manifests directory on the Kubernetes Master Node.  Set the value of --authorization-mode to Node."
    impact 0.5
    tag severity: 'medium'
    ...
    describe file('/etc/kubernetes/manifests/kube-apiserver.manifest') do
        its('content') { should match '--authorization-mode=node' }
    end
end