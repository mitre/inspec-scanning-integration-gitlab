...
control 'V-00307' do
    ...
    PKIFiles = command("ls -laR /etc/kubernetes/pki/").stdout
    
    describe 'This test can only be performed by manual examination.' do
        skip "Manual Check: Review the PKI files in Kubernetes. If the command returns any non root:root file permissions, this is a finding.\n PKI File Contents:\n #{PKIFiles}"
    end
end