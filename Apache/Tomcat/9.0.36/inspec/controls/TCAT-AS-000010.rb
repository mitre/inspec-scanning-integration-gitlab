...
control 'TCAT-AS-000010' do
  ...
  Access to the manager application must be limited and that includes the number
  of sessions allowed to access the management application. A balance must be
  struck between the number of simultaneous connections allowed to the management
  application and the number of authorized admins requiring access at any given
  time.
  ...
  system, this is not a finding.
    ...
    sudo grep -i maxactivesessions $CATALINA_HOME/webapps/manager/
  ...
  Security Plan.
    ...
    Review the maxActiveSessions setting in the $CATALINA_HOME/webapps/manager/
  ...
  tag fix_id: 'F-TCAT-AS-000010_fix'
  tag cci: ['CCI-000054']
  tag nist: ['AC-10']
