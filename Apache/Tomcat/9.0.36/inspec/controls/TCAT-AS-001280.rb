...
control 'TCAT-AS-001280' do
    title '$CATALINA_HOME/work/ folder must be owned by tomcat user, group
    tomcat.'
    ...
    command:
      ...
      sudo find $CATALINA_HOME/work -follow -maxdepth 0 \\(  ! -user tomcat -o !
    -group tomcat \\) -ls
  
      If no folders are displayed, this is not a finding.
  
      If results indicate the $CATALINA_HOME/work folder ownership and group
    membership is not set to tomcat:tomcat, this is a finding.
    '
    desc  'fix', '
      Run the following commands on the Tomcat server:
  
      sudo find $CATALINA_HOME/work -maxdepth 0 \\( ! -user tomcat \\) | sudo
    xargs chown tomcat
  
      sudo find $CATALINA_HOME/work -maxdepth 0 \\( ! -group tomcat \\) | sudo
    xargs chgrp tomcat
    '
    impact 0.5
    tag severity: 'medium'
    ...
    tag fix_id: 'F-TCAT-AS-001280_fix'
    tag cci: ['CCI-001813']
    tag nist: ['CM-5 (1)']
    
    tomcat_work_dir = file("/usr/local/tomcat/work")
    describe tomcat_work_dir do 
