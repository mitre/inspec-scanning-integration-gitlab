...
command:
  ...
  describe "Apache web server must be protected from being stopped by a non-privileged user" do 
    skip "Review the web server documentation and deployed configuration to determine 
        where the process ID is stored and which utilities are used to start/stop the web server.
        Determine where the httpd.pid file is located and verify its owner/group is of an administrative service account. 
        Verify the service utilities used to manage the Apache service is of an adminstrative service account."
    end
